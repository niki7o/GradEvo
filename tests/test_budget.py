...
def test_plan_generations_scales_inversely_with_pop_size():
    small_pop = budget_mod.plan_neat_generations(500000, population_size=20)
    large_pop = budget_mod.plan_neat_generations(500000, population_size=100)
...
def test_plan_generations_generous_enough_to_reach_budget():
    ...
    cap = budget_mod.plan_neat_generations(500000, pop, fit_eps, min_len)
    assert cap * pop * fit_eps * min_len >= 500000
    naive_central = int(np.ceil(500000 / (pop * 250 * fit_eps)))
    assert cap >= naive_central

def test_summarize_budget_reports_discrepancy():
    report = budget_mod.summarize_budget('neat', requested_steps=500000, realized_steps=[480000, 520000, 500000], wall_clock_s=[100.0, 110.0, 105.0])
    assert report.realized_steps_mean == pytest.approx(500000)
    assert report.realized_steps_min == 480000
    assert report.realized_steps_max == 520000
    assert abs(report.discrepancy_pct) < 1e-06

def test_summarize_budget_discrepancy_sign():
    over = budget_mod.summarize_budget('ppo', 100000, [110000, 110000], [1.0, 1.0])
    assert over.discrepancy_pct == pytest.approx(10.0)
    under = budget_mod.summarize_budget('ppo', 100000, [90000, 90000], [1.0, 1.0])
...
def test_budget_table_has_row_per_method():
    reports = [budget_mod.summarize_budget('ppo', 500000, [500000] * 3, [1.0] * 3), budget_mod.summarize_budget('neat', 500000, [490000] * 3, [2.0] * 3)]
