"""Orchestrates the N-seed experiment grid for both methods and conditions.

Responsibilities:

* Build clean (step-counted) training environments and clean/perturbed
  evaluation environments, with perturbation applied **only** at eval time.
* Train PPO and NEAT per seed under the shared step budget on the clean env.
* Evaluate every trained policy (and the two baselines) on both the clean and
  perturbed conditions.
* Persist tidy per-seed fitness records, learning curves, the realized-budget
  report, and trained checkpoints, plus a run-metadata JSON.

No statistics happen here; the runner only *produces* the data that
:mod:`gradevo.metrics` consumes. Heavy libraries (SB3, neat-python) are imported
lazily by the agent modules, so importing this module is cheap.
"""
...
def make_clean_env(env_id: str, seed: int, step_counted: bool = False) -> gym.Env:
    """Create a clean environment, optionally wrapped for step counting.

    Args:
        env_id: Gymnasium environment id.
        seed: Seed applied on first reset (recorded for reproducibility).
        step_counted: Whether to wrap in :class:`StepCounter` (training use).

    Returns:
        The constructed environment.

    Note:
        The environment's native ``TimeLimit`` is kept (e.g. 500 for CartPole,
        1000 for LunarLander) so fitness stays comparable to published
        baselines; ``config.MAX_EPISODE_STEPS`` is applied only as an additional
        rollout-loop safety guard, never as an override here.
    """
    env = gym.make(env_id)
    env.reset(seed=seed)
...
def make_perturbed_env(env_id: str, seed: int) -> gym.Env:
    """Create a perturbed evaluation environment (H3 held-out condition).

    Args:
        env_id: Gymnasium environment id.
        seed: Seed for the perturbation wrapper's private RNG.

    Returns:
        A :class:`PerturbationWrapper` around a fresh clean environment.
    """
    env = gym.make(env_id)
    env.reset(seed=seed)
...
def evaluate_policy(
    ...
    """Evaluate a policy over ``n_episodes`` and return per-episode returns.

    Args:
        make_env: Zero-cost env factory taking a seed.
        policy: obs->action callable.
        n_episodes: Number of evaluation episodes.
        seed: Base seed; per-episode seeds are derived deterministically.

    Returns:
        List of total episodic returns, one per episode.
    """
    env = make_env(seed)
    returns: List[float] = []
...
def mean_fitness(
    make_env: Callable[[int], gym.Env], policy: Policy, n_episodes: int, seed: int
) -> float:
    """Mean episodic return of a policy (convenience over :func:`evaluate_policy`)."""
...
@dataclass
class SeedRun:
    """One trained (method, seed) run with its clean/perturbed eval fitness.

    Attributes:
        method: ``"ppo"`` or ``"neat"``.
        seed: The run seed.
        clean_fitness: Mean fitness on the clean eval environment.
        perturbed_fitness: Mean fitness on the perturbed eval environment.
        realized_steps: Exact training step count.
        wall_clock_s: Training wall-clock time in seconds.
        curve_steps: Learning-curve step checkpoints.
        curve_fitness: Learning-curve fitness at each checkpoint.
    """
...
def _env_dims(env_id: str) -> Tuple[int, int, bool, np.ndarray, np.ndarray]:
    """Return (obs_dim, action_dim, continuous, action_low, action_high)."""
...
def train_ppo_seed(env_id: str, seed: int, run_cfg: config.RunConfig) -> SeedRun:
    """Train and evaluate one PPO seed on the clean env, eval on both conditions.
    ...
    Returns:
        A populated :class:`SeedRun`.
    """
    from gradevo.agents import ppo_agent

    clean_eval = lambda s: make_clean_env(env_id, s)  # noqa: E731
    result = ppo_agent.train_ppo(
        make_clean_env=lambda s: make_clean_env(env_id, s, step_counted=True),
        seed=seed,
        run_cfg=run_cfg,
        eval_fitness_fn=lambda pol, s: mean_fitness(clean_eval, pol, run_cfg.eval_episodes, s),
    )
...
def train_neat_seed(env_id: str, seed: int, run_cfg: config.RunConfig) -> SeedRun:
    """Train and evaluate one NEAT seed on the clean env, eval on both conditions.
    ...
    threshold = (spec.solved_threshold or 1e9) + 1e6  # keep budget the terminator
    result = neat_agent.train_neat(
        make_clean_env=lambda s: make_clean_env(env_id, s, step_counted=True),
        seed=seed,
        run_cfg=run_cfg,
        num_inputs=obs_dim,
...
def _evaluate_both_conditions(
    ...
    """Evaluate a trained policy on clean and perturbed conditions."""
    clean = mean_fitness(lambda s: make_clean_env(env_id, s), policy, run_cfg.eval_episodes, seed)
    perturbed = mean_fitness(lambda s: make_perturbed_env(env_id, s), policy, run_cfg.eval_episodes, seed + 10_000)
    ...
    return SeedRun(
        ...
        curve_fitness=list(curve_fitness),
    )
...
def evaluate_baselines(env_id: str, run_cfg: config.RunConfig) -> Dict[str, List[float]]:
    """Evaluate random and heuristic baselines over the seed set (clean env).

    Args:
        env_id: Environment id.
        run_cfg: Resolved run configuration (supplies seeds, eval episodes).

    Returns:
        Mapping ``{"random": [...], "heuristic": [...]}`` of per-seed mean
        fitness on the clean environment.
    """
    ...
    for seed in run_cfg.seeds:
        ...
        results["random"].append(
            mean_fitness(lambda s: make_clean_env(env_id, s), random_agent.act, run_cfg.eval_episodes, seed)
        )
        results["heuristic"].append(
            mean_fitness(lambda s: make_clean_env(env_id, s), heuristic_agent.act, run_cfg.eval_episodes, seed)
        )
    return results
...
def _save_ppo_model(model: object, env_id: str, seed: int) -> None:
    """Persist an SB3 PPO model to ``models/ppo``."""
...
def _save_neat_genome(genome: object, env_id: str, seed: int) -> None:
    """Persist a NEAT winner genome to ``models/neat`` via pickle."""
...
def save_results(
    ...
    """Write tidy fitness, learning-curve, budget, and metadata files.

    Args:
        env_id: Environment id (used in filenames).
        run_cfg: Resolved run configuration.
        seed_runs: All trained (method, seed) runs.
        baselines: Baseline per-seed fitness from :func:`evaluate_baselines`.

    Returns:
        Mapping of logical name to written file path.
    """
...
def _fitness_dataframe(
    env_id: str,
    seed_runs: List[SeedRun],
    baselines: Dict[str, List[float]],
    run_cfg: config.RunConfig,
) -> pd.DataFrame:
    """Build the tidy long-form fitness table."""
...
def _curves_dataframe(env_id: str, seed_runs: List[SeedRun]) -> pd.DataFrame:
    """Build the tidy long-form learning-curve table."""
    rows: List[Dict[str, object]] = []
    for run in seed_runs:
...
def _budget_dataframe(run_cfg: config.RunConfig, seed_runs: List[SeedRun]) -> pd.DataFrame:
    """Build the realized-vs-requested budget table."""
    reports = []
    for method in config.METHODS:
        ...
        if not runs:
            continue
        reports.append(
            budget_mod.summarize_budget(
                method,
                run_cfg.step_budget,
                [r.realized_steps for r in runs],
                [r.wall_clock_s for r in runs],
            )
        )
...
def _metadata(env_id: str, run_cfg: config.RunConfig) -> Dict[str, object]:
    """Assemble a run-metadata dict for reproducibility."""
    return {
        "env_id": env_id,
        "quick": run_cfg.quick,
        "n_seeds": run_cfg.n_seeds,
        "seeds": run_cfg.seeds,
        "step_budget": run_cfg.step_budget,
        "neat_population_size": run_cfg.neat_population_size,
        "eval_episodes": run_cfg.eval_episodes,
        "alpha_corrected": config.ALPHA_CORRECTED,
        "gradevo_version": __import__("gradevo").__version__,
    }
